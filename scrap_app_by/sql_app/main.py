...
# @app.post("/users/{user_id}/news_scraps", response_model=schemas.NewsScrap, description="유저가 무엇을 스크랩할지 입력할 수 있음")
# def create_scrap_for_user(user_id: str, news_scrap: schemas.NewsScrapCreate, db: Session = Depends(get_db)):
#     return crud.create_scrap_news(db=db, news_scrap=news_scrap, user_id=user_id)


# @app.get("/scraps/", response_model=List[schemas.NewsScrap], description="모든 scraps 읽어 옮")
# def read_scraps(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
#     scraps = crud.get_scraps(db, skip=skip, limit=limit)
#     return scraps
