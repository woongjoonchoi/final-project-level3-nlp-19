...
# 뉴스 기사 스크랩하기(별이)
@router.post("/{news_id}/create", description="유저아이디, 스크랩할 뉴스아이디 가져와서 db에 저장")
def post_scrap_input(request: Request, user_id: str, user_news_id: str, db: Session = Depends(get_db)):
    # Managenewsscrap Service 객체로 사용자 스크랩 정보를 DB에 저장하기   
    Managenewsscrap.create_news_scrap(db=db, user_id=user_id, user_news_id=user_news_id)
...
def delete_scrap_input(request: Request, news_scrap: schemas.NewsScrapDelete, db: Session = Depends(get_db)):
    # Managenewsscrap Service 객체로 사용자 스크랩 정보를 DB에서 삭제하기
    Managenewsscrap.delete_news_scrap(db=db, news_scrap=news_scrap)
