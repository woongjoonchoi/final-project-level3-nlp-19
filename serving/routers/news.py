...
def get_news_page(request: Request, news_id: str, user_id: str, db: Session = Depends(get_db)):
    ...
    title, article =  Newscontent.get_news(db=db, news_id=news_id)
    return templates.TemplateResponse('article_form.html', context={'request': request, 'news_id': news_id, 'user_id': user_id, 'title': title, 'article': article})
...
def post_scrap_input(request: Request, user_id: str, news_id: str, db: Session = Depends(get_db)):
    # Managenewsscrap Service 객체로 사용자 스크랩 정보를 DB에 저장하기
    Managenewsscrap.create_news_scrap(db=db, user_id=user_id, user_news_id=news_id)
    url = f'/news/{news_id}'
