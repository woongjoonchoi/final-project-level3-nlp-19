...
aiscrap_router = APIRouter(prefix="/aiscrap", tags=["AIScrap"])
...
def get_db():
    ...
    try : 
        ...
        db.close()
@aiscrap_router.post('/{owner_user_id}')
def create(request : schemas.AINewsScrap,  db : Session = Depends(get_db)):
    ...
    return new_blog
# AI scrap 페이지로 이동(웅준)
@aiscrap_router.get("/{owner_user_id}")
def get_aiscrap_page(owner_user_id ,  db : Session = Depends(get_db)):
